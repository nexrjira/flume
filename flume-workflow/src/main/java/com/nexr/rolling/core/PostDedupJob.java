...
public class PostDedupJob {
	
	private static final Log log = LogFactory.getLog(PostDedupJob.class);
	
	public static String SOURCE_PATH;
	public static String POST_MR_RESULT_PATH = "/post-result";
	public static String DEDUP_POST_PATH = "/dedup-post";
	...
	public PostDedupJob(String sourcePath) {
	...
	public boolean movePostDedupMrData() {
		boolean result = true;
		int partCnt = 0;
		FileStatus[] parts;
		Path sourcePath = new Path(POST_MR_RESULT_PATH+SOURCE_PATH); // /post-result/datatype/yyyyMMddhhmm
		try {
			...
			sourcePath = new Path(DEDUP_POST_PATH+SOURCE_PATH); // /dedup-post/datatype/yyyyMMddhhmm
			parts = fs.listStatus(sourcePath);
			for(FileStatus part : parts){
				result = fs.rename(part.getPath(), new Path(DEDUP_MR_INPUT_PATH + File.separator + part.getPath().getName()+ "_" + partCnt));
	...
	public boolean runPostDedupMr() {
		...
		try {
			...
			return ToolRunner.run(conf, new PostDedupMr(), args) == 0;
		} catch (Throwable e) {
			e.printStackTrace();
		}
		return false;
	...
	public boolean moveHourlyMrResult() {
		...
		try {
			if (!fs.exists(new Path(POST_MR_RESULT_PATH))) {
				fs.mkdirs(new Path(POST_MR_RESULT_PATH));
			}
			...
			for (FileStatus type : dataType) {
				...
				for (FileStatus hour : hours) {
					...
					for (FileStatus file : files) {
						...
						Path dest = new Path(POST_MR_RESULT_PATH
