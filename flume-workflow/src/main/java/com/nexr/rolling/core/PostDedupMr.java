...
import org.apache.hadoop.mapred.DedupPostOutputFormat;
...
public class PostDedupMr extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		JobConf jobConf = new JobConf(getConf(), PostDedupMr.class);
		jobConf.setInputFormat(SequenceFileInputFormat.class);
		jobConf.setMapperClass(IdentityMapper.class);
		jobConf.setReducerClass(UniqueKeyReduce.class);
		jobConf.setJobName("PostDedup");
		jobConf.setPartitionerClass(DedupSeqPartitioner.class);
		jobConf.setOutputFormat(DedupPostOutputFormat.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new PostDedupMr(),
	        args);
	    System.exit(res);
	  }
}
